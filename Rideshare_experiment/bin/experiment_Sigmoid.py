...
#allowable error
epsilon=10**-10
#parameters to set w_uv
alpha=18
s_taxi=25
#parameters to calculate acceptance probability
beta=1.3
...
with open('../work/learned_matrix_Sigmoid/201908_%s/A_0_08' % place, 'rb') as web:
    A_0 = pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201909_%s/A_0_09' % place, 'rb') as web:
    A_0 += pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201907_%s/A_0_07' % place, 'rb') as web:
    A_0 += pickle.load(web)
A_0+=np.eye(A_0.shape[0])
with open('../work/learned_matrix_Sigmoid/201908_%s/A_1_08' % place, 'rb') as web:
    A_1 = pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201909_%s/A_1_09' % place, 'rb') as web:
    A_1 += pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201907_%s/A_1_07' % place, 'rb') as web:
    A_1 += pickle.load(web)
A_1+=np.eye(A_0.shape[0])
with open('../work/learned_matrix_Sigmoid/201908_%s/A_2_08' % place, 'rb') as web:
    A_2 = pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201909_%s/A_2_09' % place, 'rb') as web:
    A_2 += pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201907_%s/A_2_07' % place, 'rb') as web:
    A_2 += pickle.load(web)
A_2+=np.eye(A_0.shape[0])
with open('../work/learned_matrix_Sigmoid/201908_%s/A_3_08' % place, 'rb') as web:
    A_3 = pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201909_%s/A_3_09' % place, 'rb') as web:
    A_3 += pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201907_%s/A_3_07' % place, 'rb') as web:
    A_3 += pickle.load(web)
A_3+=np.eye(A_0.shape[0])
with open('../work/learned_matrix_Sigmoid/201908_%s/A_4_08' % place, 'rb') as web:
    A_4 = pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201909_%s/A_4_09' % place, 'rb') as web:
    A_4 += pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201907_%s/A_4_07' % place, 'rb') as web:
    A_4 += pickle.load(web)
A_4+=np.eye(A_0.shape[0])
with open('../work/learned_matrix_Sigmoid/201908_%s/b_0_08' % place, 'rb') as web:
    b_0 = pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201909_%s/b_0_09' % place, 'rb') as web:
    b_0 += pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201907_%s/b_0_07' % place, 'rb') as web:
    b_0 += pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201908_%s/b_1_08' % place, 'rb') as web:
    b_1 = pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201909_%s/b_1_09' % place, 'rb') as web:
    b_1 += pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201907_%s/b_1_07' % place, 'rb') as web:
    b_1 += pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201908_%s/b_2_08' % place, 'rb') as web:
    b_2 = pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201909_%s/b_2_09' % place, 'rb') as web:
    b_2 += pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201907_%s/b_2_07' % place, 'rb') as web:
    b_2 += pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201908_%s/b_3_08' % place, 'rb') as web:
    b_3 = pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201909_%s/b_3_09' % place, 'rb') as web:
    b_3 += pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201907_%s/b_3_07' % place, 'rb') as web:
    b_3 += pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201908_%s/b_4_08' % place, 'rb') as web:
    b_4 = pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201909_%s/b_4_09' % place, 'rb') as web:
    b_4 += pickle.load(web)
with open('../work/learned_matrix_Sigmoid/201907_%s/b_4_07' % place, 'rb') as web:
    b_4 += pickle.load(web)
...
def val_calc(gamma,beta,to_am,F_i,delta,p_flag):
    if p_flag==1:
        if F_i!=0:
            if 1-(F_i+delta)>0:
                val = (gamma*to_am*np.log((F_i+delta)/(1-(F_i+delta)))*(F_i+delta)-beta*to_am*(F_i+delta)-(gamma*to_am*np.log(F_i/(1-F_i))*F_i-beta*to_am*F_i))/delta #delta流した時の1単位あたりのコスト
            else:
                val= np.infty
        else:
            if 1-(F_i+delta)>0:
                val = (gamma*to_am*np.log((F_i+delta)/(1-(F_i+delta)))*(F_i+delta)-beta*to_am*(F_i+delta))/delta
            else:
                val= np.infty
    else:
        if F_i-delta>0:
            ...
            val = (0-(gamma*to_am*np.log(F_i/(1-F_i))*F_i-beta*to_am*F_i))/delta
        else:
            val=np.infty
    return val
...
#define the function calculateing the objectives value for a given price and acceptance results
def value_eval(P,acceptance_results):
    group1 = range(n)
    group2 = range(n,n+m)
    g_post = nx.Graph()
    ...
    for (i, j) in matched_edges:
        if i>j:
            ...
            j=j-n
        opt_value+=P[i]+W[i,j]
        reward[i]=P[i].item()+W[i,j]

    return [opt_value,matched_edges,reward]

# Lists to store results
average_objective_value_proposed_list=[]
average_objective_value_MAPS_list=[]
average_objective_value_LinUCB_list=[]

computation_time_proposed_vec=[]
computation_time_MAPS_vec=[]
computation_time_LinUCB_vec=[]
    ...
    if n!=0 and m!=0:
        ...
        for i in range(df_requesters.shape[0]):
            for j in range(df_taxis.size):
                ...
                distance_ij[i,j]=distance.item()*0.001
        ...
        while delta>0.001:
            ...
            for i in range(n):
                if Cost_matrix[n+m,i]-potential[n+m]+potential[i] < -epsilon and Cap_matrix[n+m,i]>=delta:
                    ...
                    if Flow[n+m,i]!=0:
                        Cost_matrix[n+m,i]=val_calc(gamma,beta,df_requesters[i,4],Flow[n+m,i],delta,1)
                        Cost_matrix[i,n+m]=val_calc(gamma,beta,df_requesters[i,4],Flow[n+m,i],delta,-1)
        ...
        for i in range(n):
            price_proposed[i]=-gamma*df_requesters[i,4]*np.log(Flow[n+m,i]/(1-Flow[n+m,i]))+beta*df_requesters[i,4]
        computation_time_proposed=time.time()-start_time

        #start MAPS method
        #MAPS considers each area's requester to be the same.
        req_pu_loc_x_2=tu_data[df_requesters[:,1].astype('int64')-1,0].reshape((df_requesters.shape[0],1))
        req_pu_loc_y_2=tu_data[df_requesters[:,1].astype('int64')-1,1].reshape((df_requesters.shape[0],1))
        work_loc_x_2=tu_data[df_taxis.astype('int64')-1,0].reshape((df_taxis.shape[0],1))
        work_loc_y_2=tu_data[df_taxis.astype('int64')-1,1].reshape((df_taxis.shape[0],1))
        distance_ij_homogeneous=np.zeros((df_requesters.shape[0],df_taxis.size))
        for i in range(df_requesters.shape[0]):
            for j in range(df_taxis.size):
                azimuth, bkw_azimuth, distance = grs80.inv(req_pu_loc_x_2[i], req_pu_loc_y_2[i], work_loc_x_2[j],work_loc_y_2[j])
                distance_ij_homogeneous[i,j]=distance.item()*0.001
        ...
        if feasible_flag==True:
            ...
            if Nr[max_index]+1<=Nr_max[max_index]:
                ...
                #delta_new[max_index]=value_tmp-np.amin([C*p_current[r]*S[r,int(current_count[r])],D*p_current[r]])
        ...
        for i in range(df_requesters.shape[0]):
            ...
            Acceptance_probability_LinUCB[i]=1-(1/(1+np.exp((-price_LinUCB[i]+beta*df_requesters[i,4])/(gamma*df_requesters[i,4]))))
        ...
        Acceptance_probability_MAPS=1-(1/(1+np.exp(((-price_MAPS+beta*df_requesters[:,4])/(gamma*df_requesters[:,4])).astype(np.float64))))
        ...
        for k in range(num_eval):
                ...
                [opt_value_MAPS,d_post_MAPS,reward]=value_eval(price_MAPS,a_MAPS)
                objective_value_MAPS_list[k]=opt_value_MAPS
                [opt_value_LinUCB,d_post_LinUCB,reward_LinUCB]=value_eval(price_LinUCB,a_LinUCB)
                objective_value_LinUCB_list[k]=opt_value_LinUCB.item()

        average_objective_value_proposed_list.append(np.average(objective_value_proposed_list))
        average_objective_value_MAPS_list.append(np.average(objective_value_MAPS_list))
        average_objective_value_LinUCB_list.append(np.average(objective_value_LinUCB_list))

        computation_time_proposed_vec.append(computation_time_proposed)
        computation_time_MAPS_vec.append(computation_time_MAPS)
        computation_time_LinUCB_vec.append(computation_time_LinUCB)

    else:
        average_objective_value_proposed_list.append(0)
        average_objective_value_MAPS_list.append(0)
        average_objective_value_LinUCB_list.append(0)

        computation_time_proposed_vec.append(0)
        computation_time_MAPS_vec.append(0)
        computation_time_LinUCB_vec.append(0)

    print(tt_tmp+1, '/', simulataion_range, 'iterations end')

with open('../results/Average_result_Sigmoid_place=%s_day=%s_interval=%f%s.csv' %(place, day, time_interval ,time_unit), mode='w') as f:
    writer = csv.writer(f)
    writer.writerow([np.average(average_objective_value_proposed_list),np.average(computation_time_proposed_vec)])
    writer.writerow([np.average(average_objective_value_MAPS_list),np.average(computation_time_MAPS_vec)])
    writer.writerow([np.average(average_objective_value_LinUCB_list),np.average(computation_time_LinUCB_vec)])
