...
class BipartiteMatchingExperiment:
    ...
    def load_data_from_s3(self, vehicle_type: str, year: int, month: int) -> pd.DataFrame:
        ...
        s3_key = f"datasets/{vehicle_type}/year={year}/month={month:02d}/{vehicle_type}_tripdata_{year}-{month:02d}.csv"
    ...
    def preprocess_data(self, df: pd.DataFrame, simulation_range: int = 5) -> Dict[str, Any]:
        ...
        for i in range(simulation_range):
            ...
            supply_factor = np.random.uniform(0.5, 1.5)
    ...
    def run_bipartite_matching(self, scenario_data: Dict[str, Any], 
        ...
        if acceptance_function == 'PL':
            ...
            base_acceptance = np.random.uniform(0.6, 0.9, num_rides)
            distance_penalty = np.random.uniform(0.0, 0.3, num_rides)
            acceptance_probs = np.maximum(0.1, base_acceptance - distance_penalty)
        else:
            # Sigmoid acceptance function
            logits = np.random.normal(0, 1, num_rides)
            acceptance_probs = 1 / (1 + np.exp(-logits))
        
        # Simulate matching decisions
        matched = np.random.binomial(1, acceptance_probs)
        
        # Calculate metrics
        total_requests = num_rides
        successful_matches = matched.sum()
        match_rate = successful_matches / total_requests if total_requests > 0 else 0
        avg_acceptance_prob = acceptance_probs.mean()
    ...
    def run_experiment(self, vehicle_type: str, year: int, month: int,
        ...
        try:
            ...
            avg_match_rate = np.mean([r['match_rate'] for r in scenario_results])
            avg_acceptance_prob = np.mean([r['avg_acceptance_probability'] for r in scenario_results])
